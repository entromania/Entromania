...
<li class="active"><a href="<%=request.getContextPath()%>/matchCo?flag=viewmatch">View Match</a></li>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body no-padding">
					<table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
						...
						<tbody>
						<c:forEach items="${sessionScope.viewmatch}" var="m">
							<tr class="odd gradeX">
								<td>${m.m_id}</td>
								<td>${m.title}</td>
								<td>${m.date}</td>
								<td>${m.time}</td>
								<td>${m.stadiumvo.stadium}</td>
								<td>${m.m_des}</td>
								<td>
								<a href="<%=request.getContextPath()%>/matchCo?id=${m.m_id}&flag=deletematch"><button class="btn-primary btn">Delete</button></a> | 
								<a href="<%=request.getContextPath()%>/matchCo?id=${m.m_id}&flag=editmatch"><button class="btn-primary btn">Edit</button></a>
								</td>
							   </tr>
						</c:forEach>
						</tbody>
