				...
				<div class="panel-body">
					<div class="tab-content">
						<div class="tab-pane active" id="horizontal-form">
							<form name="stadiumForm" class="form-horizontal" action="<%=request.getContextPath()%>/conticketCo" method="post">
							<%@taglib uri="http://java.sun.com/jstl/core_rt" prefix="c"%>
							<c:forEach var="a" items="${sessionScope.editcticket}">
							<div class="form-group mb-n">
									<input type="hidden" value="${a.c_ticketId}" name="ticketId">
									...
									<c:forEach items="${sessionScope.loadstadium}" var="s">
									<c:if test="${a.stadiumvo.s_id eq s.s_id }"><option selected="selected" value="${s.s_id}">${s.stadium}</option></c:if>
										<c:if test="${a.stadiumvo.s_id ne s.s_id }"><option value="${s.s_id}">${s.stadium}</option></c:if>
									</c:forEach>
									</select>
									</div>
							</div> <br>
							<div class="form-group mb-n">
								    <label for="largeinput" class="col-sm-2 control-label label-input-lg">Select Concert</label>
									<div class="col-sm-8">
									<select name="concertid" id="concertid" class="form-control">
									<c:forEach items="${sessionScope.loadconcert}" var="m">
									<c:if test="${a.concertvo.cn_id eq m.cn_id }"><option selected="selected" value="${m.cn_id}">${m.title}</option></c:if>
										<c:if test="${a.concertvo.cn_id ne m.cn_id }"><option value="${m.cn_id}">${m.title}</option></c:if>
									</c:forEach>
									</select>
									</div>
							</div> <br>
								<div class="form-group mb-n">
									<label for="largeinput" class="col-sm-2 control-label label-input-lg">Ticket Name</label>
									<div class="col-sm-8">
										<input type="text" name="ticketname" class="form-control input-lg" value="${a.ticketname}" id="largeinput" placeholder="Enter Ticket Name" required>
									</div>
								 </div> <br>
								 <div class="form-group mb-n">
									<label for="largeinput" class="col-sm-2 control-label label-input-lg">Ticket Price</label>
									<div class="col-sm-8">
										<input type="text" name="ticketprice" class="form-control input-lg" id="largeinput" value="${a.ticketprice}" placeholder="Enter Ticket Price" required>
									</div>
								 </div> 
								<br/>
								<div class="form-group mb-n">
									<label for="largeinput" class="col-sm-2 control-label label-input-lg">Description</label>
									<div class="col-sm-8">
										<textarea name="ticketdesc"  value="${a.ticketdesc}" class="form-control autosize" required></textarea>
									</div>
								 </div> <br/>
								
					<div class="row">
						<div class="col-sm-8 col-sm-offset-2">
							<button class="btn-primary btn" name="flag" value="updateticket">Update Ticket</button>
