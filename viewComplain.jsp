			...
			<div class="panel panel-default">
				...
				<div class="panel-body no-padding">
					<table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
						...
						<tbody>
						<c:forEach items="${sessionScope.viewComplain}" var="t">
							<tr class="odd gradeX">
								<td>${t.complainId}</td>
								<td>${t.complainTitle}</td>
								<td>${t.complainDescription}</td>
								<td>${t.complainDate }</td>
								<td class="center">${t.complainTime }</td>
								<td class="center">${t.loginVoTo.email }</td>
								<td class="center">${t.loginVoFrom.email }</td>
								<td>
								<a href="<%=request.getContextPath()%>/complainCo?flag=reply&id=${t.complainId}"><button class="btn-primary btn">Reply</button></a> 
								</td>
							</tr>
						</c:forEach>
						</tbody>
