		...
		<div id="layout-static">
			...
			<div class="static-content-wrapper">
				<div class="static-content">
												...
												<div class="tab-content">
													<div class="tab-pane active" id="horizontal-form">
														<form class="form-horizontal"
															action="<%=request.getContextPath()%>/ProductCo"
															method="post" enctype="multipart/form-data">
															<%@taglib uri="http://java.sun.com/jstl/core_rt"
																prefix="c"%>
															<c:forEach items="${sessionScope.editProduct}" var="p">
															...
															<input type="hidden" name="productId" value="${p.productId}">
																<label for="largeinput"
																	...
																	Category</label>
																<div class="col-sm-8">
																	<select name="categoryId" id="selector1" class="form-control">
																		<c:forEach items="${sessionScope.loadCategory}"
																			var="s">
																			<c:if test="${p.categoryVo.categoryId eq s.categoryId}"><option selected="selected" value="${s.categoryId}">${s.categoryName}</option></c:if>
																			<c:if test="${p.categoryVo.categoryId ne s.categoryId}"><option value="${s.categoryId}">${s.categoryName}</option></c:if>
																		</c:forEach>
																	</select>
																</div>
															</div>
															...
															<div class="form-group mb-n">
																<label for="largeinput"
																	...
																	SubCategory</label>
																<div class="col-sm-8">
																	<select name="subCategoryId" id="selector1" class="form-control">
																		<c:forEach items="${sessionScope.loadSubCategory}"
																			var="z">
																		<c:if test="${p.subcategoryVo.subcategoryId eq z.subcategoryId}"><option selected="selected" value="${z.subcategoryId}">${z.subcategoryName}</option></c:if>
																		<c:if test="${p.subcategoryVo.subcategoryId ne z.subcategoryId}"><option value="${z.subcategoryId}">${z.subcategoryName}</option></c:if>
																		</c:forEach>
																	</select>
																</div>
															</div>
															...
															<div class="form-group mb-n">
																<label for="largeinput"
																	class="col-sm-2 control-label label-input-lg">Product
																	Name</label>
																<div class="col-sm-8">
															...
															<div class="form-group mb-n">
																<label for="largeinput"
																	...
																	Price</label>
																<div class="col-sm-8">
																	<input type="text" name="productPrice" value="${p.productPrice}"
																		class="form-control input-lg" id="largeinput"
					...
					<div class="col-sm-8">
						<img height="100px" width="100px" src="${pageContext.request.contextPath }/doc/${p.attachmentVo.encryptedFileName}">
										...
										<input type="file" name="productImage">
															...
															<div class="form-group">
																<label for="col-sm-2 mb-n"
																	...
																	Desription</label>
																<div class="col-sm-8">
																	<textarea name="productDescription" class="form-control autosize"
																		required>${p.productDescription}</textarea>
																</div>
															</div>
															<br />
															</c:forEach>
															<div class="row">
																<div class="col-sm-8 col-sm-offset-2">
																	<button class="btn-primary btn">Add Product</button>
																	<input type="hidden" name="flag" value="updateProduct">
																</div>
															</div>
